...
if (isset($_POST["id"]) && !empty($_POST["id"])) {
    ...
    if (empty($input_location)) {
        $address_err = "Please enter a location.";
    } else {
    ...
    if (empty($input_productssold)) {
        $sproductssold_err = "Please enter the productssold amount.";
    ...
    if (empty($name_err) && empty($location_err) && empty($productssold_err)) {
        ...
        $sql = "UPDATE employees SET name=?, location=?, productssold=? WHERE id=?";

        if ($stmt = mysqli_prepare($link, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssi", $param_name, $param_location, $param_productssold, $param_id);
